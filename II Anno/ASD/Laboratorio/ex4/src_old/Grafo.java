...
*/
  ...
  /*
   for (Arch<E> archInList : getArchList(arch.getSorgente()))
      if (archInList.equals(arch))
        return true;

    if(!diretto)
      for (Arch<E> archInList : getArchList(arch.getDestinazione()))
        if (archInList.equals(arch.reveArch()))
          return true;
