...
int stampaGiornaliera()
{
    if (var->setOnStampaGiornaliera == FALSE)
        return 0;
    int i, j;
    ...
    for (i = 0; i < SO_MERCI; ++i) {
        for (j = 0, presente_navi = 0, scaduta_navi = 0, merce_persa = 0; j < var->n_navi; ++j) {
            ...
            if (navi[j].pidNavi != -1)
                presente_navi += navi[j].stiva[i];
            else
                merce_persa += navi[j].stiva[i];
        }
    ...
    for (i = 0, j = 0; i < var->n_navi; ++i) {
        if (navi[i].pidNavi != -1) {
            if (navi[i].carica == TRUE && navi[i].in_mare == TRUE)
                ++in_mare_cariche;
            if (navi[i].carica == FALSE && navi[i].in_mare == TRUE)
                ++in_mare_no_cariche;
            if (navi[i].in_mare == FALSE)
                ++in_porto;
            if (navi[i].rallentata_da_tempesta > 0)
...
int stampaFinale()
{
    ...
    fprintf(out_progetto, "║└───────╜\n");
    fprintf(out_progetto, "║ ┌─────────────────────────────────────────────────────────────────────┐\n");
    for (i = 0, max = 0; i < var->n_porti; ++i)
        if (porti[i].merce_offerta > max)
            max = porti[i].merce_offerta;
    for (i = 0; i < var->n_porti; ++i)
        if (porti[i].merce_offerta == max)
            fprintf(out_progetto, "║ │ Porto %3d ha offerto la quantità maggiore di merce (%8d ton)   │\n", i + 1, max);
    fprintf(out_progetto, "║ ├─────────────────────────────────────────────────────────────────────┤\n");
    for (i = 0, max = 0; i < var->n_porti; ++i)
        if (porti[i].merce_ricevuta > max)
            max = porti[i].merce_ricevuta;
    for (i = 0; i < var->n_porti; ++i)
        if (porti[i].merce_ricevuta == max)
            fprintf(out_progetto, "║ │ Porto %3d ha ricevuta la quantità maggiore di merce (%8d ton)  │\n", i + 1, max);
    fprintf(out_progetto, "║ ├──────────────────────────────────────────────────────────────┬──────┤\n");
    for (i = 0, j = 0; i < var->n_porti; ++i)
        j += porti[i].colpito_da_mareggiata;
...
int stampa(int flagStampa)
{
    switch (flagStampa) {
    case INIZIALE:
        return stampaIniziale();
    case GIORNALIERA:
        return stampaGiornaliera();
    case FINALE:
        return stampaFinale();
    default:
        return -1;
...
void attShm()
{
    ...
    if (var->flagTerminazione == 0) {
        if ((merci = shmat(shmMerci, NULL, 0)) == (Merce*)-1)
            ERROR;
        if ((porti = shmat(shmPorti, NULL, 0)) == (Porto*)-1)
            ERROR;
...
void createIPCs(char* file)
{
    ...
    if ((shmVar = shmget(ftok(FTOK_FILE, 'v'), sizeof(Var), IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((var = shmat(shmVar, NULL, 0)) == (void*)-1)
        ERROR;
    ...
    if ((shmPorti = shmget(ftok(FTOK_FILE, 'p'), sizeof(Porto) * var->n_porti, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((shmNavi = shmget(ftok(FTOK_FILE, 'n'), sizeof(Nave) * var->n_navi, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((shmMerci = shmget(ftok(FTOK_FILE, 'm'), sizeof(Merce) * SO_MERCI, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((shmPila = shmget(ftok(FTOK_FILE, 'i'), sizeof(int) * (var->n_navi + 1), IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((semShm = semget(ftok(FTOK_FILE, 'a'), 1, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((semPidNavi = semget(ftok(FTOK_FILE, 'f'), 1, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((semPila = semget(ftok(FTOK_FILE, 's'), 2, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((msgPila = msgget(ftok(FTOK_FILE, 'g'), IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((semMareggiata = semget(ftok(FTOK_FILE, 'x'), var->n_porti, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((banchine = semget(ftok(FTOK_FILE, 'z'), var->n_porti, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
    if ((semProcessi = semget(ftok(FTOK_FILE, 't'), 1, IPC_CREAT | IPC_EXCL | PERMISSIONS)) == -1)
        ERROR;
...
void deallocIPCs()
{
    ...
    reserveSem(semShm, 0);
    printf("::::::::::::::::::::::::::::::::::::::\n");
    if (shmctl(shmVar, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  shmVar        :   deallocati  :::\n");
    if (shmctl(shmPorti, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  shmPorti      :   deallocati  :::\n");
    if (shmctl(shmNavi, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  shmNavi       :   deallocati  :::\n");
    if (shmctl(shmMerci, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  shmMerci      :   deallocati  :::\n");
    if (shmctl(shmPila, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  shmPila       :   deallocati  :::\n");
    if (semctl(semShm, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  semShm        :   deallocati  :::\n");
    if (semctl(semPidNavi, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  semPidNavi    :   deallocati  :::\n");
    if (semctl(semPila, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  semPila       :   deallocati  :::\n");
    if (semctl(banchine, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  banchine      :   deallocati  :::\n");
    if (msgctl(msgPila, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  msgPila       :   deallocati  :::\n");
    if (semctl(semMareggiata, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  semMareggiata :   deallocati  :::\n");
    if (semctl(semProcessi, IPC_RMID, 0) == -1) {
        ERROR;
    } else
        printf(":::  semProcessi   :   deallocati  :::\n");
    printf("::::::::::::::::::::::::::::::::::::::\n");
    return;
}
...
void loadIPCs()
{
    if ((shmVar = shmget(ftok(FTOK_FILE, 'v'), sizeof(Var), PERMISSIONS)) == -1)
        ERROR;
    if ((var = shmat(shmVar, NULL, 0)) == (void*)-1)
        ERROR;
    if ((shmPorti = shmget(ftok(FTOK_FILE, 'p'), sizeof(Porto) * var->n_porti, PERMISSIONS)) == -1)
        ERROR;
    if ((shmNavi = shmget(ftok(FTOK_FILE, 'n'), sizeof(Nave) * var->n_navi, PERMISSIONS)) == -1)
        ERROR;
    if ((shmMerci = shmget(ftok(FTOK_FILE, 'm'), sizeof(Merce) * SO_MERCI, PERMISSIONS)) == -1)
        ERROR;
    if ((shmPila = shmget(ftok(FTOK_FILE, 'i'), sizeof(int) * (var->n_navi + 1), PERMISSIONS)) == -1)
        ERROR;
    if ((msgPila = msgget(ftok(FTOK_FILE, 'g'), PERMISSIONS)) == -1)
        ERROR;
    if ((semPila = semget(ftok(FTOK_FILE, 's'), 2, PERMISSIONS)) == -1)
        ERROR;
    if ((semShm = semget(ftok(FTOK_FILE, 'a'), 1, PERMISSIONS)) == -1)
        ERROR;
    if ((semPidNavi = semget(ftok(FTOK_FILE, 'f'), 1, PERMISSIONS)) == -1)
        ERROR;
    if ((semMareggiata = semget(ftok(FTOK_FILE, 'x'), var->n_porti, PERMISSIONS)) == -1)
        ERROR;
    if ((banchine = semget(ftok(FTOK_FILE, 'z'), var->n_porti, PERMISSIONS)) == -1)
        ERROR;
    if ((semProcessi = semget(ftok(FTOK_FILE, 't'), 1, PERMISSIONS)) == -1)
        ERROR;
