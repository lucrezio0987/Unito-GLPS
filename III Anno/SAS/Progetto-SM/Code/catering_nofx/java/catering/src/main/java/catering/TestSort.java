...
public class TestSort {
    public static void main(String[] args) throws UseCaseLogicException {
        ...
        Duty croissant = new Duty(
                "Croissant",
                "Pasta sfoglia francese burrosa e croccante.",
                5,
                4,
                180,
                12,
        ...
        Duty maionese = new Duty(
                "Maionese",
                "Maionese per gli hamburger.",
                2,
                3,
                10,
                1,
        ...
        CatERing.getInstance().getSheetMgr().addJob("Torta Croissant", true, false, croissant);
        CatERing.getInstance().getSheetMgr().addJob("Torta Bignè", true, false, bigne);
        CatERing.getInstance().getSheetMgr().addJob("Torta Maionese", true, false, maionese);
        ...
        ArrayList<Job> sortedJobs_importance = CatERing.getInstance().getSheetMgr().sortJobs("IMPORTANZA");
        for (Job job: sortedJobs_importance) {
            if (job.getDuty().getImportance() > 0)
                System.out.println("job: " + job.getTitle() + ", importanza: " + job.getDuty().getImportance());
        }

        System.out.println("\nCompiti ordinati per difficoltà:\n");
        ArrayList<Job> sortedJobs_difficult = CatERing.getInstance().getSheetMgr().sortJobs("DIFFICOLTA");
        for (Job job: sortedJobs_difficult) {
            if (job.getDuty().getDifficult() > 0)
                System.out.println("job: " + job.getTitle() + ", difficoltà: " + job.getDuty().getDifficult());
        }
    }
}
